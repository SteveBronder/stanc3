...
let rec expand_arg = function
  ...
  | DDeepVectorized ->
      let all_base =
        [UnsizedType.UInt; UReal; UComplex; URowVector; UVector; UMatrix]
      in
      List.(
...
let vector_types =
  [ UnsizedType.UReal; UArray UReal; UComplex; UArray UComplex; UVector
  ; URowVector ]
  ...
  List.iter
    ~f:(fun i ->
      List.iter
        ...
        [ UnsizedType.UArray UInt; UArray UReal; UArray UComplex; UVector
        ; URowVector; UMatrix ] )
    (List.range 0 8) ;
  List.iter
    ~f:(fun i ->
      ...
      List.iter
        ~f:(fun j ->
          add_unqualified
            ...
            , [bare_array_type (j, i); bare_array_type (j, i)] ) )
        [ UnsizedType.UArray UReal; UArray UComplex; UVector; URowVector
        ; UMatrix ] )
    (List.range 0 8) ;
  List.iter
    ~f:(fun i ->
  ...
  List.iter
    ~f:(fun i ->
      List.iter
        ...
        (List.range 0 8) )
    [ UnsizedType.UArray UReal; UnsizedType.UArray UComplex; UVector; URowVector
    ; UMatrix ] ;
  List.iter
    ~f:(fun i ->
      List.iter
...
let add_binary_vec_real_int name =
  ...
  List.iter
    ~f:(fun i ->
      add_unqualified
        ( name
        , ReturnType (bare_array_type (UComplex, i))
        , [UComplex; bare_array_type (UInt, i)] ) )
    (List.range 0 8)
...
let () =
  ...
  add_unqualified ("abs", ReturnType UComplex, [UComplex]) ;
  List.iter
  ...
  add_unqualified ("assign_multiply", Void, [UComplex; UComplex]) ;
  ...
  add_unqualified ("log_sum_exp", ReturnType UComplex, [UArray UComplex]) ;
  ...
  add_unqualified ("max", ReturnType UComplex, [UArray UComplex]) ;
  ...
  add_unqualified ("mean", ReturnType UComplex, [UArray UComplex]) ;
  ...
  add_unqualified ("min", ReturnType UComplex, [UArray UComplex]) ;
  ...
  add_unqualified ("multiply", ReturnType UComplex, [UReal; UComplex]) ;
  ...
  add_unqualified ("sum", ReturnType UComplex, [UArray UComplex]) ;
